...
it("add kledingstuk test2", () => {
  ...
  it("maak kledingstuk met verkeerde merk", () => {
      ...
      cy.get('[data-cy="type_input"]').type("broek");
      cy.get('[data-cy="submit_kledingstuk"]').click();
      cy.get('[data-cy="brand_input2"]').find('.ant-form-item-explain').should('have.text', 'Vul een merk in!');

  });
  it("show kledingstukken", () => {
      cy.intercept('GET', 'http://localhost:3000/api/kledingstukken', {fixture: 'kledingstukken.json',},);
      cy.visit('http://localhost:3000/kleren');
      cy.get('[data-cy="kledinglijst"').find("tbody").find(".ant-table-row-level-0").first({force:true}).contains("td", "Converse");
      cy.get('[data-cy="kledinglijst"').find("tbody").find(".ant-table-row-level-0").first({force:true}).contains("td", "55");
  });
